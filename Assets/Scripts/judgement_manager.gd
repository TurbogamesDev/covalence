...
static func get_judgement_from_max_offset_table_and_offset(max_offset_table: Dictionary[Enums.JUDGEMENT_TYPE, float], offset: float):
...
func calculate_judgement_for_press(note_type: Enums.NOTE_TYPE, offset: float) -> Enums.JUDGEMENT_TYPE:
    if note_type == Enums.NOTE_TYPE.REGULAR_NOTE:
        return get_judgement_from_max_offset_table_and_offset(MAXIMUM_OFFSET_FOR_JUDGEMENT_PRESS_1, offset)

    elif note_type == Enums.NOTE_TYPE.HOLD_NOTE:
        return get_judgement_from_max_offset_table_and_offset(MAXIMUM_OFFSET_FOR_JUDGEMENT_PRESS_1, offset)

    return Enums.JUDGEMENT_TYPE.NONE

func calculate_judgement_for_release(note_type: Enums.NOTE_TYPE, offset: float) -> Enums.JUDGEMENT_TYPE:
    if note_type == Enums.NOTE_TYPE.HOLD_NOTE:
        return get_judgement_from_max_offset_table_and_offset(MAXIMUM_OFFSET_FOR_JUDGEMENT_RELEASE_1, offset)
